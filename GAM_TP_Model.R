...
mutate(Ecotope=as.factor(Ecotope),Month=month((Date),abbr=TRUE,label=TRUE),Day=yday((Date))) %>%
drop_na(TPO4)
...
filter(Position=="Downstream",STA=="STA-3/4 Cell 2B",Ecotope!="Naiad")
...
#Multivariate Models
GAM_Outflow_DCS_DWN <- gam(TPO4 ~s(Outflow)+s(DCS),method="REML",select=TRUE,data =TP_GAM_DATA_DWN)  #ecotope and outflow
...
GAM_DCS_Ecotope_DWN <- gam(TPO4 ~Ecotope+s(DCS,by=Ecotope),method="REML",select=TRUE,data =TP_GAM_DATA_DWN) #ecotope and water column random effects
GAM_Day_DCS_Outflow_Temp_RI_DWN <- gam(TPO4 ~s(Day)+s(DCS)+s(Outflow)+s(Temp),method="REML",data =TP_GAM_DATA_DWN)  #Random Intercept
GAM_Day_Ecotope_DCS_Outflow_Temp_RI_DWN_log_link <- gam(TPO4 ~Ecotope+s(Day)+s(DCS)+s(Outflow)+s(Temp),method="REML",family=Gamma(link="log"),data =TP_GAM_DATA_DWN)  #Random Intercept
GAM_Day_Ecotope_DCS_Outflow_Temp_RI_DWN_normal <- gam(TPO4 ~Ecotope+s(Day)+s(DCS)+s(Outflow)+s(Temp),method="REML",data =TP_GAM_DATA_DWN)  #Random Intercept
GAM_Day_Ecotope_DCS_Outflow_Temp_RS_DWN <- gam(TPO4 ~Ecotope+s(Day)+s(Day,by=Ecotope,bs="fs", m=1)+s(DCS)+s(Outflow)+s(Temp),method="REML",data =TP_GAM_DATA_DWN)  #Random smooth
GAM_Ecotope_DCS_Outflow_Temp_DWN <- gam(TPO4 ~Ecotope+s(DCS)+s(Outflow)+s(Temp),method="REML",data =TP_GAM_DATA_DWN)  #Random Intercept
GAM_DCS_Outflow_Temp_DWN <- gam(TPO4 ~s(DCS)+s(Outflow)+s(Temp),method="REML",data =TP_GAM_DATA_DWN)  #Random Intercept
...
summary(GAM_Outflow_DCS_DWN)  #Inflow deviance explained 48.8%
plot(GAM_Outflow_DCS_DWN, shade = TRUE, pages = 1, scale = 0, seWithMean = TRUE,all.terms=TRUE)
concurvity(GAM_Outflow_DCS_DWN)
gam.check(GAM_Outflow_DCS_DWN, k.rep = 1000)
...
summary(GAM_DCS_Ecotope_DWN )  #Inflow deviance explained 39% 
plot(GAM_DCS_Ecotope_DWN , shade = TRUE, pages = 1, scale = 0, seWithMean = TRUE,all.terms=TRUE)
concurvity(GAM_DCS_Ecotope_DWN)
gam.check(GAM_DCS_Ecotope_DWN, k.rep = 1000)

summary(GAM_DCS_Outflow_Temp_DWN  )  #Inflow deviance explained 64.9%
plot(GAM_DCS_Outflow_Temp_DWN  , shade = TRUE, pages = 1, scale = 0, seWithMean = TRUE,all.terms=TRUE)
concurvity(GAM_DCS_Outflow_Temp_DWN )
gam.check(GAM_DCS_Outflow_Temp_DWN, k.rep = 1000)

summary(GAM_Ecotope_DCS_Outflow_Temp_DWN )  #Inflow deviance explained 66.8%
plot(GAM_Ecotope_DCS_Outflow_Temp_DWN, shade = TRUE, pages = 1, scale = 0, seWithMean = TRUE,all.terms=TRUE)
concurvity(GAM_Ecotope_DCS_Outflow_Temp_DWN )
gam.check(GAM_Ecotope_DCS_Outflow_Temp_DWN, k.rep = 1000)
draw(GAM_Ecotope_DCS_Outflow_Temp_DWN,all.terms=TRUE)
...
summary(GAM_DCS_DWN)  #DCS deviance explained 37.8%
plot(GAM_DCS_DWN, shade = TRUE, pages = 1, scale = 0, seWithMean = TRUE)
...
summary(GAM_Day_DCS_Outflow_Temp_RI_DWN)  #DCS deviance explained 85.2%
plot(GAM_Day_DCS_Outflow_Temp_RI_DWN, shade = TRUE, pages = 1, scale = 0, seWithMean = TRUE,all.terms=TRUE)

summary(GAM_Day_Ecotope_DCS_Outflow_Temp_RI_DWN )  #DCS deviance explained 87.4%
plot(GAM_Day_Ecotope_DCS_Outflow_Temp_RI_DWN , shade = TRUE, pages = 1, scale = 0, seWithMean = TRUE,all.terms=TRUE)
gam.check(GAM_Ecotope_DCS_Outflow_Temp_DWN, k.rep = 1000)
...
summary(GAM_Day_Ecotope_DCS_Outflow_Temp_RI_DWN_normal )  #DCS deviance explained 52.8%
plot(GAM_Day_Ecotope_DCS_Outflow_Temp_RI_DWN , shade = TRUE, pages = 1, scale = 0, seWithMean = TRUE,all.terms=TRUE)
gam.check(GAM_Ecotope_DCS_Outflow_Temp_DWN, k.rep = 1000)


summary(GAM_Day_Ecotope_DCS_Outflow_Temp_RI_DWN_log_link)  #DCS deviance explained 74.2%
plot(GAM_Day_Ecotope_DCS_Outflow_Temp_RI_DWN_log_link , shade = TRUE, pages = 1, scale = 0, seWithMean = TRUE,all.terms=TRUE)
mgcv::gam.check(GAM_Day_Ecotope_DCS_Outflow_Temp_RI_DWN_log_link,pages=1 )
draw(GAM_Day_Ecotope_DCS_Outflow_Temp_RI_DWN_log_link, term="Ecotope",parametric=TRUE,smooth_col="orange",ci_col="orange",scales = "fixed")& Presentation_theme2
...
GAM_Ecotope_DCS_Outflow_Temp_DWN_plot <-smooth_estimates(GAM_Ecotope_DCS_Outflow_Temp_DWN) %>%
add_confint()

Residuals <- TP_GAM_DATA_DWN %>%
add_partial_residuals(GAM_Ecotope_DCS_Outflow_Temp_DWN)

filter(smooth == "s(x2)") 
  
ggplot(GAM_Ecotope_DCS_Outflow_Temp_DWN_plot) +facet_wrap(~smooth,scales="free")+
#geom_rug(aes(x = x2),
#           data = eg1,
...
concurvity(GAM_Ecotope_DCS_Outflow_Temp_DWN )
gam.check(GAM_Ecotope_DCS_Outflow_Temp_DWN, k.rep = 1000)
draw(GAM_Ecotope_DCS_Outflow_Temp_DWN)
smooth_estimates(GAM_Ecotope_DCS_Outflow_Temp_DWN)
...
draw(filter(GAM_Ecotope_DCS_Outflow_Temp_DWN,smooth=="s(Outflow)"),all.terms=TRUE)
p & theme_bw()  

outflow <- draw(GAM_Ecotope_DCS_Outflow_Temp_DWN, select = "s(Outflow)",smooth_col="yellow",ci_col="yellow")& Presentation_theme
DCS<- draw(GAM_Ecotope_DCS_Outflow_Temp_DWN, select = "s(DCS)",smooth_col="orange",ci_col="orange")
Temp <- draw(GAM_Ecotope_DCS_Outflow_Temp_DWN, select = "s(Temp)",smooth_col="green",ci_col="green")
ecotope <- draw(GAM_Ecotope_DCS_Outflow_Temp_DWN, term="Ecotope",parametric=TRUE,smooth_col="orange",ci_col="orange")& Presentation_theme
  
Ecotope_GAM_Plot<- draw(GAM_Ecotope_DCS_Outflow_Temp_DWN, term="Ecotope",parametric=TRUE,smooth_col="orange",ci_col="orange",scales = "fixed")& Presentation_theme2

ggsave(plot = last_plot(),filename="./Figures/Ecotope_GAM_Plot.jpeg",width =13.333, height =7.5, units = "in")
gamOut(summary(GAM_Ecotope_DCS_Outflow_Temp_DWN),"./Figures/GAM model summary.csv") 
