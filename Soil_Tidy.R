...
filter(Ecotope!="Naiad",TEST_NAME %in% c("Magnesium","Aluminum","Iron","Calcium","Carbon","Nitrogen","Phosphorus","AFDW")) %>%
group_by(Ecotope,MATRIX,TEST_NAME) %>%
summarise(Samples=n(),`Mean`=mean(VALUE,na.rm=T),`SD`=sd(VALUE,na.rm=T),SE=sd(VALUE,na.rm=T)/n()^.5) %>%
mutate(`Units`=case_when(TEST_NAME %in% c("Carbon","AFDW")~"%",TRUE~"(mg/kg)")) %>%
...
group_by(TEST_NAME,Units) %>% 
mutate(y=max(Mean)+max(SE))   %>% #add y coordinate to display labels
...
#Run ANOVA and Tukey's HSD on RAS vs Floc for soil TP
RAS_vs_Floc_TP <- Soils_data %>%
filter(Ecotope!="Naiad", TEST_NAME %in% c("Phosphorus")) %>%  
nest_by(Ecotope,TEST_NAME) %>% 
mutate(Model = list(lm(VALUE ~ MATRIX, data = data))) %>%   #create model
mutate(Tukey_HSD=list(TukeyHSD(aov(Model)))) %>%             #Tukey's pairwise comparison
mutate(Letters=list(multcompLetters4(Model, Tukey_HSD)$MATRIX$Letters)) %>% #Label ecotopes that are not sig different with same letter
mutate(Letters2=list(as.list(Letters)))  %>%                                 #create list that can be unnested. Multicomplist will not unnest.  
unnest_wider(Letters2) %>%
pivot_longer(names_to="MATRIX",values_to="Letter",7:8) %>%
select("Ecotope","MATRIX","TEST_NAME","Letter")  %>% 	
mutate(Units=case_when(TEST_NAME %in% c("AFDW","Carbon")~"%",TRUE~"(mg/kg)"))
